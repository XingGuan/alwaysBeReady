...
```   
> 提示：使用`flex`和`grid`布局可以帮助元素更好地适应视口变化，但单独使用`CSS`不能完全解决遮挡问题。   

##### 解决方案2：`JavaScript`自动滚动到可视区域   
当输入框获得焦点时，使用`JavaScript`将其滚动到可视区域  
```javascript
function onInputFocus(event) { 
  const input = event.target; // 延迟执行以确保软键盘已弹出 
  setTimeout(() => { 
...
```
> 提示：这种方法在大多数现代浏览器中有效，`setTimeout`延迟可以确保软键盘完全弹出后再执行滚动操作。   

##### 解决方案3：监听窗口大小变化   
检测窗口大小变化（软键盘弹出会改变窗口高度），动态调整布局。   
```javascript
let initialHeight = window.innerHeight;   
window.addEventListener('resize',()=>{
  const currentHeight = window.innerHeight;
  // 如果当前高度小于初始高度，则认为软键盘弹出  
...
```
> 这种方法可以处理大多数情况，但某些浏览器可能有特殊行为，需要进行测试。    

##### 最佳实践建议  
结合使用多种方法可以提供最可靠的解决方案：  
```html  
// 1.设置正确的视口`meta`标签  
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">   
// 2.使用弹性布局    
// 3.添加`JavaScript`滚动逻辑  
// 4.监听`resize`事件作为后备方案   
// 5.对于固定底部元素，可以使用以下`CSS`;  
.fixed-element{
  position:fixed;  
  bottom:0;  
  width:100%;
...
```  
> 综合使用`CSS`布局和`JavaScript`检测，可以在大多数设备上有效解决软键盘遮挡问题。   

 #### 完整的解决方案 
 ```javascript 
// 保存初始高度  
let initialHeight = window.innerHeight
let isResizing = false; // 标记是否正在处理resizing 事件，避免重复处理
// 处理窗口大小变化 
function handleResize() {
    if (isResizing) return;
    isResizing = true;
    const currentHeight = window.innerHeight;
    //如果高度减少超过100px，认为是键盘弹出  
    if (currentHeight < initialHeight - 100) {
        const activeElement = document.activeElement;
        // `document.activeElement` 是一个非常有用的属性，它用于获取当前文档（`document`）中**获得了焦点（`focus`）**的元素。 
        // 哪个元素正在接收键盘输入事件，`document.activeElement`获取到的就是哪个元素。   
        if (activeElement && (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA')) {
            setTimeout(() => {
                activeElement.scrollIntoView({
                    block: 'center',
...
// 页面加载时也记录一次高度，防止加载过程中尺寸发生变化  
window.addEventListener("load",()=>{
    initialHeight = window.innerHeight;
})
```
> 这个增强版解决方案添加了防抖机制。避免`resize`事件频繁触发导致的性能问题，同时增加了高度变化的阈值检测，减少误判的可能性。    
