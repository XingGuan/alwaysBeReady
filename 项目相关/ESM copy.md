...
## 二、为什么需要 ESM？没它的时候，大家都怎么玩的？（历史方案）
...
- `import {}`：精准导入，性能最好，**首选**。
- `import * as`：整包导入，方便但容易让打包体积变大，**慎用**。
- `import()`：动态导入，配合箭头函数实现懒加载，**按需使用**。

ESM 现在已经是现代前端开发的基石了。它用一套统一的、语言原生的标准，取代了之前社区里各种五花八门的方案。说到底，它就是 JavaScript 官方的 `import/export` 语法，理解它的设计思路和使用场景，对理解整个前端工程化体系都很有帮助。
